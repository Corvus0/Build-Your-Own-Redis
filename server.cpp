...
static bool try_one_request(Conn *conn) {
    ...
    int32_t err = do_request(
        &conn->rbuf[4 + conn->rbuf_read], len,
    ...
    if (conn->wbuf_size + 4 + len > sizeof(conn->wbuf)) {
    ...
    memcpy(&conn->wbuf[4 + conn->wbuf_size], &rescode, 4);
...
int main() {
    ...
    while (true) {
        ...
        int rv = epoll_wait(epollfd, events, epoll_args.size() + 1, 1000);
        if (rv < 0) {
            die("epoll_wait()");
        }
