...
static inline uint32_t min(size_t lhs, size_t rhs) {
...
// find node with value closest to score
static AVLNode *zset_find_closest(ZSet *zset, double score, AVLNode **found) {
    AVLNode *cur = zset->tree;
    while (cur) {
        ZNode *z_cur = container_of(cur, ZNode, tree);
        *found = cur;
        if (z_cur->score < score) {
            cur = cur->right;
        } else if (z_cur->score > score) {
            cur = cur->left;
        } else {
            break;
        }
...
// returns number of elements within a [start, stop] range
uint64_t zset_range(ZSet *zset, double start, double stop) {
    if (start > stop) {
        return 0;
    }
    AVLNode *left = NULL;
    AVLNode *right = NULL;
    AVLNode *left_bound = zset_find_closest(zset, start, &left);
    AVLNode *right_bound = zset_find_closest(zset, stop, &right);
    return (right_bound || left_bound) ? avl_rank(right) - avl_rank(left) + 1: 0;
}
