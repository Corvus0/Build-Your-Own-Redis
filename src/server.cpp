...
static struct {
    HMap db;
    // a map of all client connetions, keyed by fd
    std::vector<Conn *> fd2conn;
    // times for idle connections
    DList idle_list;
    // timers for TTLs
...
// dispose of the entry after it is attached from the key space
...
static void do_zrank(std::vector<std::string> &cmd, std::string &out) {
    ...
    const std::string &name = cmd[2];
    ZNode *znode = zset_lookup(ent->zset, name.data(), name.size());
    return znode ? out_int(out, avl_rank(&znode->tree)) : out_nil(out);
}
...
static void do_zrange(std::vector<std::string> &cmd, std::string &out) {
    ...
    double start = 0;
    double stop = 0;
    if (!str2dbl(cmd[2], start)) {
        return out_err(out, ERR_ARG, "expect dbl");
    }
    if (!str2dbl(cmd[3], stop)) {
        return out_err(out, ERR_ARG, "expect dbl");
    }
    return out_int(out, zset_range(ent->zset, start, stop));
}
...
static bool try_one_request(Conn *conn) {
    ...
    // flush buffer before overflow
    if (conn->wbuf_size + 4 + wlen > sizeof(conn->wbuf)) {
    ...
    memcpy(&conn->wbuf[4 + conn->wbuf_size], out.data(), out.size());
    ...
    return (conn->state == STATE_REQ || conn->wbuf_size);
}
...
static inline bool hnode_same(HNode *lhs, HNode *rhs) {
    return lhs == rhs;
}
...
int main() {
    ...
    while (true) {
        ...
        for (Conn *conn : g_data.fd2conn) {
            ...
            if (epoll_ctl(epollfd, EPOLL_CTL_ADD, conn->fd, &ev) == -1) {
                die("epoll ctl add");
            }
            epoll_args.push_back(ev);
        }
        epoll_event events[epoll_args.size() + 1];
        memset(events, 0, (epoll_args.size() + 1) * sizeof(epoll_event));

        // poll for active fds
        int timeout_ms = (int)next_timer_ms();
        int rv = epoll_wait(epollfd, events, epoll_args.size() + 1, timeout_ms);
        if (rv < 0) {
            die("epoll_wait()");
        }

        // deregister fds from set before any are closed
        for (epoll_event ev : epoll_args) {
